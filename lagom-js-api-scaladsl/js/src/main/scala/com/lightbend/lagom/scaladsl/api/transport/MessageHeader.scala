...
sealed trait MessageHeader {
  ...
  def getHeader(name: String): Option[String] = {
    headerMap.get(name.toLowerCase).flatMap(_.headOption).map(_._2)
  }
  ...
  def getHeaders(name: String): immutable.Seq[String] = {
    headerMap.get(name.toLowerCase).fold(immutable.Seq.empty[String])(_.map(_._2))
  }
...
object RequestHeader {
  ...
  def apply(
    ...
    RequestHeaderImpl(method, uri, protocol, acceptedResponseProtocols, principal, headers.groupBy(_._1.toLowerCase))
  ...
  private case class RequestHeaderImpl(
    ...
    override def withHeaders(headers: immutable.Seq[(String, String)]): RequestHeaderImpl =
      copy(headerMap = headers.groupBy(_._1.toLowerCase))
    override def withHeader(name: String, value: String): RequestHeaderImpl =
      copy(headerMap = headerMap + (name.toLowerCase -> immutable.Seq(name -> value)))
    ...
    override def removeHeader(name: String): RequestHeaderImpl =
      copy(headerMap = headerMap - name.toLowerCase)
  }
}
...
object ResponseHeader {
  def apply(
      ...
      protocol: MessageProtocol,
      headers: immutable.Seq[(String, String)]
  ): ResponseHeader = ResponseHeaderImpl(status, protocol, headers.groupBy(_._1.toLowerCase))
