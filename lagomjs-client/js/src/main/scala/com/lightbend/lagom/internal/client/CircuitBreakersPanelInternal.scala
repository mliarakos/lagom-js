...
private[lagom] class CircuitBreakersPanelInternal(
  ...
  private val breakers = mutable.Map.empty[String, Option[CircuitBreakerHolder]]
  ...
  private def createCircuitBreaker(id: String): Option[CircuitBreakerHolder] = {
    val allExceptionAsFailure: Try[_] => Boolean = {
  ...
  private def breaker(id: String): Option[CircuitBreakerHolder] =
    breakers.getOrElseUpdate(id, createCircuitBreaker(id))
}
